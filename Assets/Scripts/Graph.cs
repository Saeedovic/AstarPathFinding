...
public class Graph : MonoBehaviour
{
    ...
    List<Node> GetNeighbors(int x, int y, Node[,] nodeArray, Vector2[] directions)
    {
        ...
        foreach (Vector2 dir in directions)
        {
            ...
            if(IsWithinBounds(newX, newY) && nodeArray[newX, newY] != null && nodeArray[newX, newY].nodeType != NodeType.Closed)
            {
    ...
    public float GetNodeDistance(Node source, Node target)
    {
        ...
        int dy = Mathf.Abs(source.yIndex - target.yIndex); // Im just getting the absloute value because im unsure and the value might be negative 
        ...
        return (1.4f * diagonalSteps + straightSteps);// 24
    }
